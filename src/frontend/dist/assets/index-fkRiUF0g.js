...
function playTone(ctx, freq, duration, type = "square", vol = 0.18) {
  if (!ctx) return;
  try {
...
function playNoise(ctx, duration, vol = 0.15) {
  if (!ctx) return;
  try {
...
function drawPlayer(ctx, x, alive, tick) {
  if (!alive) return;
...
function drawHUD(ctx, score, hiScore, wave, lives, playerX) {
  ctx.fillStyle = "#111";
...
function bulletHitsShields(bx, by, shields, isAlien) {
  for (const sh of shields) {
    for (let r = 0; r < SHIELD_ROWS_N; r++) {
      for (let c = 0; c < SHIELD_COLS_N; c++) {
        ...
        if (bx >= px && bx <= px + SHIELD_PX && by >= py && by <= py + SHIELD_PX) {
          ...
          for (let dr = -spread; dr <= spread; dr++) {
            for (let dc = -spread; dc <= spread; dc++) {
              const nr = r + dr;
              const nc = c + dc;
              if (nr >= 0 && nr < SHIELD_ROWS_N && nc >= 0 && nc < SHIELD_COLS_N) {
...
function alienMarchInterval(aliveCount, wave) {
...
function SpaceInvaders() {
  ...
  const gsRef = reactExports.useRef(initialState());
  const keysRef = reactExports.useRef(/* @__PURE__ */ new Set());
  const audioRef = reactExports.useRef(null);
  const rafRef = reactExports.useRef(0);
  const startGame = reactExports.useCallback((gs) => {
    const hi = gs.hiScore;
  ...
  const nextWave = reactExports.useCallback((gs) => {
    gs.wave++;
    ...
    if (gs.screen === "PLAYING") {
        ...
        if (gs.bullet.y < HUD_H) {
          ...
          if (bulletHitsShields(gs.bullet.x, gs.bullet.y, gs.shields, false)) {
            ...
            for (const alien of gs.aliens) {
              ...
              if (gs.bullet.x >= alien.x - 4 && gs.bullet.x <= alien.x + ALIEN_W + 4 && gs.bullet.y >= alien.y - 4 && gs.bullet.y <= alien.y + ALIEN_H + 4) {
                alien.alive = false;
                gs.bullet.active = false;
                ...
                spawnExplosion(
                  ...
                  alien.x + ALIEN_W / 2,
                  alien.y + ALIEN_H / 2,
                  alien.type === 0 ? "#ff88ff" : alien.type === 1 ? "#88ffff" : "#aaff44",
                  14
                );
                gs.popups.push({
                  x: alien.x + ALIEN_W / 2,
            ...
            if (!hit && gs.ufo.active) {
              if (Math.abs(gs.bullet.x - gs.ufo.x) < 28 && Math.abs(gs.bullet.y - UFO_Y) < 14) {
                gs.ufo.active = false;
                gs.bullet.active = false;
                gs.score += gs.ufo.pts;
                if (gs.score > gs.hiScore) {
      ...
      for (const alien of aliveAliens) {
        if (alien.y + ALIEN_H >= PLAYER_Y - 10) {
          gs.screen = "GAME_OVER";
          playNoise(audioRef.current, 1, 0.3);
          break;
        }
      }
      ...
      for (const ab of gs.alienBullets) {
        ...
        if (gs.playerAlive && Math.abs(ab.x - gs.playerX) < PLAYER_W / 2 + 4 && ab.y >= PLAYER_Y - 4 && ab.y <= PLAYER_Y + PLAYER_H + 4) {
          ab.active = false;
          gs.lives--;
          gs.flashTick = 30;
          spawnExplosion(
      ...
      if (gs.ufoCooldown <= 0 && !gs.ufo.active) {
        ...
        gs.ufo.pts = [50, 100, 150, 200, 250, 300][Math.floor(Math.random() * 6)];
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx(SpaceInvaders, {})
    }
  );
}
